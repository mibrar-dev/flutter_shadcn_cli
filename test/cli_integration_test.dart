  ...
  group('CLI integration', () {
    ...
    test('json flag works in any position for json-enabled commands', () async {
      ...
      final registriesResult = await _runCli(
        cwd: appRoot.path,
        args: ['--advanced', '--offline', 'registries', '--json'],
      );
    ...
    test(
      ...
      () async {
        final manifestRegistry = Directory(
          p.join(tempRoot.path, 'manifest_registry', 'registry'),
              ...
              'components': [
                {
                  ...
                  'import':
                      'package:app/ui/shadcn/components/button/button.dart',
    ...
    test(
      'theme help hides import flags unless advanced mode is enabled',
      () async {
    ...
    test(
      'theme --apply-file installs a declarative manifest in advanced mode',
      () async {
    ...
    test('developer registry override requires advanced mode', () async {
      final result = await _runCli(
        cwd: appRoot.path,
        args: ['list', '--registry-path', registryRoot.path],
      );

      expect(result.exitCode, ExitCodes.usage);
      expect(
        result.stderr,
        contains('--registry-path flag requires --advanced'),
      );
    });
    ...
    test(
      ...
      () async {
        ...
        await cli.main(['add', 'shadcn:button']);
    ...
    test(
      ...
      () async {
        ...
        await cli.main(['add', '@shadcn/button', '--include-files=preview']);
    ...
    test(
      'default command sets default namespace and registries list works',
      () async {
    ...
    test(
      'init installs registry default components after inline actions',
      () async {
        final registriesPath = _writeRegistriesFile(appRoot, [
          {
            'id': 'shadcn_entry',
    ...
    test(
      ...
      () async {
        ...
        File(p.join(registryRoot.path, 'components', 'index.json'))
          ..createSync(recursive: true)
          ..writeAsStringSync(
            jsonEncode({
              'files': ['registry/components/button/button.dart'],
    ...
    test(
      ...
      () async {
        ...
        await cli.main(['--advanced', '--offline', 'init', '--yes']);
    ...
    test(
      ...
      () async {
                ...
                'registries': [
                  {
                    ...
                    'init': {
                      ...
                      'actions': [
                        ...
                        {'type': 'mergePubspec', 'deriveFlutterAssets': true},
                      ],
                    },
                  },
                ],
        ...
        expect(
          File(
            p.join(appRoot.path, 'assets', 'fonts', 'typography_fonts.otf'),
          ).existsSync(),
          isTrue,
        );
        expect(
          File(
            p.join(appRoot.path, 'assets', 'theme', 'typography_fonts.dart'),
          ).existsSync(),
          isFalse,
        );
        final pubspec = File(
    ...
    test(
      'assets and remove use inline registry actions when available',
      () async {
        final server = await HttpServer.bind(InternetAddress.loopbackIPv4, 0);
        addTearDown(() async {
          await server.close(force: true);
                    ...
                    'init': {
                      ...
                      'actions': [
                        ...
                        },
                        {
                          ...
                          'files': [
                            'registry/shared/fonts/typography_fonts.dart',
                          ],
                        },
                        {
                          'type': 'mergePubspec',
    ...
    test(
      'assets fails instead of installing removed asset component fallback',
      () async {
        File(p.join(appRoot.path, '.shadcn', 'config.json')).writeAsStringSync(
          jsonEncode({
            'defaultNamespace': 'shadcn',
    ...
    test('add exits schema invalid when components schema fails', () async {
      final invalidRegistry = Directory(
        p.join(tempRoot.path, 'invalid_add_schema'),
      )..createSync(recursive: true);
    ...
    test(
      'add bypasses invalid components schema with skip integrity',
      () async {
        final invalidRegistry = Directory(
          p.join(tempRoot.path, 'invalid_add_schema_bypass'),
        )..createSync(recursive: true);
        _writeRegistryFixtures(invalidRegistry);
        _writeSchemaRequiringBlockedField(invalidRegistry);
    ...
    test('init exits schema invalid when components schema fails', () async {
      final invalidRegistry = Directory(
        p.join(tempRoot.path, 'invalid_init_schema'),
      )..createSync(recursive: true);
      _writeRegistryFixtures(invalidRegistry);
      _writeSchemaRequiringBlockedField(invalidRegistry);
      final registriesFile = _writeRegistriesFile(appRoot, [
    ...
    test('preloaded commands map schema failures to schema invalid', () async {
      final invalidRegistry = Directory(
        p.join(tempRoot.path, 'invalid_preload_schema'),
      )..createSync(recursive: true);
      _writeRegistryFixtures(invalidRegistry);
      _writeSchemaRequiringBlockedField(invalidRegistry);
    ...
    test('validate reports schema failures', () async {
      final invalidRegistry = Directory(
        p.join(tempRoot.path, 'invalid_registry'),
      )..createSync(recursive: true);
      File(
        p.join(invalidRegistry.path, 'components.json'),
    ...
    test('offline list uses cached index', () async {
      ...
      final registryId = registryUrl.replaceAll(
      ...
      File(
        p.join(cacheDir.path, 'index.json'),
      ).writeAsStringSync(jsonEncode({'components': []}));
    ...
    test(
      'default command accepts --registries-path for local registries.json',
      () async {
        final localRegistriesFile = File(
          p.join(appRoot.path, 'dev_registry', 'registries.json'),
        )..createSync(recursive: true);
    ...
    test(
      'theme widget commands fail explicitly without runtime execution',
      () async {
...
void _writeRegistryFixtures(Directory registryRoot) {
  ...
  final componentsDir = Directory(
    p.join(root, 'registry', 'components', 'button'),
  )..createSync(recursive: true);
  ...
  final sharedUtilDir = Directory(p.join(root, 'registry', 'shared', 'util'))
    ..createSync(recursive: true);
  final sharedColorExtensionsDir = Directory(
    p.join(root, 'registry', 'shared', 'color_extensions'),
  )..createSync(recursive: true);
  final sharedFormControlDir = Directory(
    p.join(root, 'registry', 'shared', 'form_control'),
  )..createSync(recursive: true);
  final sharedFormValueSupplierDir = Directory(
    p.join(root, 'registry', 'shared', 'form_value_supplier'),
  )..createSync(recursive: true);
  final typographyDir = Directory(
    p.join(root, 'registry', 'components', 'typography_fonts'),
  )..createSync(recursive: true);
  final iconDir = Directory(
    p.join(root, 'registry', 'components', 'icon_fonts'),
  )..createSync(recursive: true);
  final stepperDir = Directory(
    p.join(root, 'registry', 'components', 'stepper'),
  )..createSync(recursive: true);

  File(
    p.join(componentsDir.path, 'button.dart'),
  ).writeAsStringSync('class Button {}');
  File(p.join(componentsDir.path, 'README.md')).writeAsStringSync('# Button');
  File(
    p.join(componentsDir.path, 'meta.json'),
  ).writeAsStringSync('{"id":"button"}');
  File(
    p.join(componentsDir.path, 'preview.dart'),
  ).writeAsStringSync('class ButtonPreview {}');
  File(
    p.join(componentsDir.path, 'preview_state.dart'),
  ...
  File(
    p.join(dialogDir.path, 'dialog.dart'),
  ).writeAsStringSync('class Dialog {}');
  File(
    p.join(dialogDir.path, 'meta.json'),
  ).writeAsStringSync('{"id":"dialog"}');

  File(
    p.join(sharedThemeDir.path, 'theme.dart'),
  ).writeAsStringSync('class ThemeHelper {}');
  File(
    p.join(sharedUtilDir.path, 'util.dart'),
  ).writeAsStringSync('class UtilHelper {}');
  File(
    p.join(sharedColorExtensionsDir.path, 'color_extensions.dart'),
  ).writeAsStringSync('class ColorExtensions {}');
  File(
    p.join(sharedFormControlDir.path, 'form_control.dart'),
  ).writeAsStringSync('class FormControl {}');
  File(
    p.join(sharedFormValueSupplierDir.path, 'form_value_supplier.dart'),
  ).writeAsStringSync('class FormValueSupplier {}');

  File(
    p.join(typographyDir.path, 'typography_fonts.dart'),
  ).writeAsStringSync('class TypographyFonts {}');
  File(
    p.join(typographyDir.path, 'meta.json'),
  ).writeAsStringSync('{"id":"typography_fonts"}');
  File(
    p.join(iconDir.path, 'icon_fonts.dart'),
  ).writeAsStringSync('class IconFonts {}');
  File(
    p.join(iconDir.path, 'meta.json'),
  ).writeAsStringSync('{"id":"icon_fonts"}');

  File(
    p.join(stepperDir.path, 'stepper.dart'),
  ).writeAsStringSync('class Stepper {}\nclass Step {}');
  File(
    p.join(stepperDir.path, 'meta.json'),
        ...
        'files': [
          {
            ...
            'destination':
                '{sharedPath}/color_extensions/color_extensions.dart',
          },
        ],
      },
      {
      ...
      },
      {
        ...
        'files': [
          ...
          },
          {
            ...
            'destination':
                '{installPath}/components/typography_fonts/meta.json',
          },
        ],
        'shared': [],
        'dependsOn': [],
        ...
        'files': [
          {
            ...
            'destination':
                '{installPath}/components/icon_fonts/icon_fonts.dart',
          },
          {
  ...
  for (final component
    ...
    File(
      p.join(root, 'registry', 'components', id, 'meta.json'),
    ).writeAsStringSync(const JsonEncoder.withIndent('  ').convert(component));
  }

  File(
    p.join(registryRoot.path, 'components.json'),
  ).writeAsStringSync(const JsonEncoder.withIndent('  ').convert(registryJson));
  File(
    p.join(registryRoot.path, 'components.schema.json'),
...
String _writeRegistriesFile(
  ...
  file.writeAsStringSync(
    jsonEncode({'schemaVersion': 1, 'registries': entries}),
  );
  return file.path;
}
...
void _writePubspec(Directory targetRoot) {
  ...
  File(
    p.join(targetRoot.path, 'pubspec.yaml'),
