...
class InstallerDryRunService {
  ...
  final InstallerConfigResolver configResolver;
  final CliLogger logger;

  Future<DryRunPlan> buildPlan(
    ...
    for (final component in resolved.values) {
      ...
      for (final file in component.files) {
        final destination = _resolveComponentDestination(file);
  ...
  void printPlan(DryRunPlan plan) {
    ...
    if (plan.fonts.isNotEmpty) {
      ...
      for (final font in plan.fonts) {
        ...
        for (final fontAsset in font.fonts) {
          final weight = fontAsset.weight != null
  ...
  String _resolveComponentDestination(RegistryFile file) {
    final installPath = configResolver.installPath(config);
  ...
  String _resolveDestinationPath(String destination) {
    final variables = {
      'installPath': configResolver.installPath(config),
      'sharedPath': configResolver.sharedPath(config),
    };
