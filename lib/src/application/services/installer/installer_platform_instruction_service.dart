...
class InstallerPlatformInstructionService {
  InstallerPlatformInstructionService({
  ...
  Future<void> applyPlatformInstructions(
    Component component,
    ShadcnConfig? config,
  ) async {
  ...
  Future<void> writePlatformSection({
    ...
    final block = formatPlatformBlock(fullPath, marker, lines);
    await file.writeAsString(existing + block);
    logger.detail('Updated $targetPath ($platform/$section)');
  }

  String formatPlatformBlock(String path, String marker, List<String> lines) {
